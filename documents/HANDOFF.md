...
- Application/docs head `690b1afde743a751408fa15ca87bdffcf5681f23` produced Vercel preview deployment `dpl_fSBbPtuiSJvNkEcnqJVE3Fey39TW`, state READY.
- The repo's `npm run build` command runs the audit MCP tests, the new stack-planner contract, existing release contracts, and the Next production build; the READY deployment therefore includes the stack-planner release gate on that head.
- `scripts/verify-stack-planner.mjs` protects the five buyer goals, three catalog-backed capability slots per goal, real listing handoff, acquisition events, starter-kit attribution, homepage entry point, and sitemap route.
...
- After an approved release, distribute the planner through qualified MCP-directory traffic and observe planner completion → starter-kit intent. Do not infer sales from page views or feature completion.
