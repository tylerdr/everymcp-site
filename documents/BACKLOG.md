# Backlog

*Single source of truth for what's next. Coding agents update at session end. Tyler can add at any time.*

---

## Active Sprint
- [ ] T001: Stripe test-mode checkout and download receipt — run a labeled no-charge sandbox payment and retry the download.
- [ ] T002: Analytics/GSC proof — verify Vercel event receipt and submit/confirm the canonical sitemap through the root-owned properties.
- [ ] T007: Audit MCP activation — merge reviewed implementation, independently verify the exact durable Vercel edge rate-limit rule, then enable only after provider receipts are current.

## Next Up
- [ ] T003: [Task name]
- [ ] T004: [Task name]

## Tech Debt
- [ ] TD001: Durable inquiry storage and operator notification — manual mailto remains a fallback and is not automated lead delivery.
- [ ] TD002: Human implementation/sponsor fulfillment — keep those offers inquiry-only until a configured provider/workflow can deliver them.
- [ ] TD003: GetFoundInChat MCP firewall readiness receipt and BrandKit executable tool/auth contract — provider calls remain pending until each exact production contract is independently verified.
- [ ] TD004: Authenticated portfolio audits — do not expose until a real caller identity, tenant scope, authorization policy, and abuse controls are implemented.

## Open Questions (blocking work)
- [ ] OQ001: Which approved Stripe account/key should be used for a labeled test-mode payment and production starter-kit sales? — blocks T001

## Open Questions (non-blocking)
- [ ] OQ002: What provider and operator identity should back durable inquiry capture and notifications? — answer before restoring web submissions

## Completed
- [x] T008: Implement bounded single-site audit MCP contract, tests, readiness endpoint, and activation gate — PR #10 merged and exact production behavior verified 2026-09-23; endpoint remains setup-pending.
- [x] T006: Source-backed marketplace directory, listing checklists, and official brand visual pass — done 2026-09-22. External submissions and done-for-you fulfillment remain closed.
- [x] T005: Raster social preview and favicon compatibility — done 2026-09-22.
- [x] T000: Replace false-success lead capture with honest manual inquiry — done 2026-09-20.
- [x] T004: Add server-verified $49 starter-kit checkout and deterministic download — done 2026-09-20.

2026-09-20: Replaced false-success lead capture with honest manual inquiry. Durable storage, operator notification and delivered-email proof remain open.
